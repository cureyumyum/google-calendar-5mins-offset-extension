...
### 1. 連続会議がもたらす「ムダなストレス」の解消

連続した会議（例: 10:00-11:00, 11:00-12:00）は、**休憩や移動時間がなく**、参加者に大きなストレスを与えます。

- 前の会議が長引き、次の会議に定刻で集合できない
- 遅れる側は「すみません、遅れます」とチャットする**心理的負担**
- 待つ側は「あと1分待つべきか？」「前の予定はなにか？」と**余計な確認作業**が発生する
...
### 💡 なぜ「開始を遅らせる」のか？（設計思想の核心）

会議の**終了時刻を5分前倒し**にするケース（例: 10:00〜10:55）も考えられます。

しかし、このルールでは以下の理由から<b>「開始時刻を5分遅らせる」</b>アプローチを採用しました。

これは、**区切りの良い時刻**（00分/30分）に終わることで、参加者全員の終了時刻への意識が高まり、**時間厳守**が促進されやすいと考えたためです。

| 項目 | 終了5分前倒し (10:00〜10:55) | 開始5分遅延 (10:05〜11:00) |
| --- | --- | --- |
| **終了時刻** | 10:55, 11:25 など | **11:00, 11:30 など** |
| **会議の延長** | 「あと5分ある」という心理が働きやすく、次の会議開始（11:00）を無視して延長しがちになる。 | 終了時刻が11:00/11:30で区切られているため、**「次の会議の開始時刻は11:05/11:35だ」**という認識が生まれ、延長に対する意識的・心理的ハードルが高まる。 |
...
Googleカレンダーで「10:00」の枠をクリックして「10:05」に直し、終了時刻を「10:30」に直す。この<b>「毎日発生する小さな手間」</b>を解決し、手間なく運用ルールを定着させる目的でこの拡張機能を開発しました。
